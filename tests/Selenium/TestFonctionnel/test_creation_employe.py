...
def creer_employe(driver):
    """Crée un nouvel employé après la connexion"""
    try:
        # Générer des données aléatoires pour l'employé
        matricule = f"EMP{random.randint(10000, 99999)}"
        nom = f"Test{random.randint(100, 999)}"
        prenom = f"Prenom{random.randint(100, 999)}"
        employe_email = f"test.{nom.lower()}{random.randint(1, 999)}@example.com"
        ...
        # Remplir le formulaire en s'appuyant sur la structure du document HTML
        # Matricule
        ...
        champ_email.clear()
        champ_email.send_keys(employe_email)
        print("Email saisi")
        
        # Service (select2)
        try:
            # Cliquer sur le sélecteur de service pour ouvrir le dropdown
            driver.execute_script("""
                document.querySelector("#service-take").dispatchEvent(new Event('mousedown'));
            """)
            time.sleep(1)
            
            # Sélectionner la première option non vide
            service_options = driver.find_elements(By.CSS_SELECTOR, "#service-take option:not(:first-child)")
            if service_options:
                service_id = service_options[0].get_attribute('value')
                driver.execute_script(f"""
                    document.querySelector("#service-take").value = "{service_id}";
                    document.querySelector("#service-take").dispatchEvent(new Event('change'));
                """)
        ...
        # Fonction (select2)
        try:
            # Cliquer sur le sélecteur de fonction pour ouvrir le dropdown
            driver.execute_script("""
                document.querySelector("#fonction-take").dispatchEvent(new Event('mousedown'));
            """)
            time.sleep(1)
            
            # Sélectionner la première option non vide
            fonction_options = driver.find_elements(By.CSS_SELECTOR, "#fonction-take option:not(:first-child)")
            if fonction_options:
                fonction_id = fonction_options[0].get_attribute('value')
                driver.execute_script(f"""
                    document.querySelector("#fonction-take").value = "{fonction_id}";
                    document.querySelector("#fonction-take").dispatchEvent(new Event('change'));
                """)
        ...
        # Déclencher un événement pour s'assurer que la valeur est prise en compte
        driver.execute_script("""
        ...
        # Vérifier si l'employé a été créé avec succès (présence d'un message de succès)
        page_source = driver.page_source
        if "Succès" in page_source or nom in page_source or matricule in page_source:
            print(f"Création de l'employé '{nom} {prenom}' réussie")
            return True, f"{nom} {prenom}"
        else:
            print(f"Impossible de vérifier la création de l'employé '{nom} {prenom}'")
...
def main():
    ...
    print("Démarrage du test de création d'employé")
    ...
    try:
        # Étape 1: Connexion
        if login(driver):
            print("Connexion réussie, passage à la création d'employé")
            
            # Étape 2: Création d'employé
            success, nom_employe = creer_employe(driver)
            
            if success:
                print(f"Test réussi: Employé '{nom_employe}' créé avec succès")
            else:
                print(f"Test échoué: Impossible de créer l'employé")
        else:
